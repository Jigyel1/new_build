...
# rubocop:disable Layout/LineLength
{
  ...
  management: 'The Management can view projects and get an overview of the projects in the pipeline. Additionally users with that role will make decisions if they are required.',
  administrator: 'The Administrator can edit roles, make changes to users and invite new users to the tool. He also can change parameters in the admin toolkit. The Admin cannot handle any project.',
  manager_commercialization: 'Users with that role will take up certain Marketing-activities that cannot be performed by the NBO / KAM users.',
  kam: 'Users with that role handle projects that are marked as Key Accounts.',
  manager_nbo_kam: 'Users with that role get an overview on the tasks of NBO & KAM users. They will also make certain decisions if required.',
  ...
  team_expert: 'Users with this role can analyze Standard projects, issue contracts and handle all administrative tasks of Newbuild Projects. (analysis – Offer – Verification – Commercialisation)',
  team_standard: 'Users with this role can issue contracts after the projects are analyzed und can handle the administrative tasks of Newbuild Projects. (Offer – Verification – Commercialization)'
