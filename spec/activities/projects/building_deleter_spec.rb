  ...
  describe '.activities' do
    context 'as an owner' do
      it 'returns activity in terms of first person' do
        ...
        expect(activities.dig(0, :displayText)).to eq(
          t('activities.projects.building_deleted.owner', project_name: project.name, name: building.name)
        )
      end
    end
    ...
    context 'as a general user' do
      ...
      it 'returns activity text in terms of a third person' do
        ...
        expect(activities.size).to eq(1)
        expect(activities.dig(0, :displayText)).to eq(
          t('activities.projects.building_deleted.others', project_name: project.name, name: building.name, owner_email: super_user.email)
        )
      end
    end
  end
